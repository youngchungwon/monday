...
DALI_REPORT_BOARD_ID = 5027259250
DALI_SUB_COST_COL_ID = "numeric_mm1jrkf0"          # 비용
...
# 5) 데일리 서브아이템 읽기
# =========================
def get_dali_subitem_info(subitem_id: int) -> Dict[str, Any]:
    query = f"""
    query ($id: [ID!]) {{
      items(ids: $id) {{
        id
        name
        board{{ id }}
        column_values(ids: ["{DALI_SUB_COST_COL_ID}", "{DALI_SUB_LINK_MAIN_COL_ID}", "{DALI_DONE_COL_ID}"]) {{
          id
          text
          value
          ... on BoardRelationValue {{
    ...
    """
    data = monday_query(query, {"id": [str(subitem_id)]})
    sub = (data.get("items") or [None])[0]
    if not sub:
        raise RuntimeError(f"Dali subitem {subitem_id} not found")
    ...
    for cv in sub.get("column_values") or []:
        ...
        if cid == DALI_SUB_COST_COL_ID:
            txt = (cv.get("text") or "").strip()
            try:
                cost = int(float(txt)) if txt else 0
            except ValueError:
                cost = 0

        elif cid == DALI_SUB_LINK_MAIN_COL_ID:
            linked = cv.get("linked_item_ids") or []
            if linked:
                main_item_id = int(linked[0])  # 1개 연결 가정

        elif cid == DALI_DONE_COL_ID:
            v = cv.get("value")
            if v:
                try:
                    done = json.loads(v).get("checked", False)
                except Exception:
                    done = False
...
# 5) 메인보드 서브아이템 생성
# =========================
DALI_MAIN_SUBITEM_NAME = "법인카드 지출내역"

def create_dali_subitem_on_main(main_item_id: int, info: Dict[str, Any]) -> int:
    col_vals = {
        MAIN_SUB_COST_COL_ID: info["cost"],
    }

    mutation = """
    mutation ($parent_item_id: ID!, $item_name: String!, $column_values: JSON!) {
      create_subitem(parent_item_id: $parent_item_id, item_name: $item_name, column_values: $column_values) { id }
    }
    """
    vars = {
        "parent_item_id": str(main_item_id),
        "item_name": DALI_MAIN_SUBITEM_NAME,
        "column_values": json.dumps(col_vals, ensure_ascii=False),
    }
    data = monday_query(mutation, vars)
...
def monday_webhook():
    ...
    try:
        # 1) 먼저 "어느 보드의 서브아이템인지" 파악
        # (간단히: Dali용 query를 먼저 해보고 board_id 확인해도 되고,
